<%
if session("utype")<>9 then
	response.Write("请先用管理员登录!")
	response.end
end if
dim apsw:apsw="!hbsgcc"     '★登录密码，您需要修改，留空可取消登录密码★
dim url:url="admin_file.asp"   '★本程序文件名，默认为index.asp,最好不要重命名本程序文件名★
dim exename
exename="bmp,jpg,gif,png,txt,rar,zip,doc,xls,css,html,swf,mdb,asp,js"     '★允许上传的文件类型
...
%>
<div id=menu>
<a href='<%=url%>?fileurl=<%=server.urlencode(rootpath)%>&show=show'>站点根目录</a>&nbsp;&nbsp;
