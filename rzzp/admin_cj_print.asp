...
<%
Response.AddHeader "Content-Disposition", "attachment;filename=岗位报名笔试成绩表.xls" 
...
tmpgid=0
 while not rs1.eof
	...
	sql="select * from tb_cj where jid="&jid&" and sfz in(select card from tb_user where uid in(select uid from tb_bm where jid="&jid&" and gid="&gid&" and hg2)) order by  cj1 desc"
	rs.open sql,conn,1,1
	while not rs.eof
	if tmpgid<>gid then
		tmpgid=gid
