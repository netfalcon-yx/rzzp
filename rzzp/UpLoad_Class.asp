...
Class AnUpLoad
	...
	Private Sub Class_Initialize()
		...
		vVersion = "AienAspUpload V13.01.16"
		vMaxSize = -1
		vSingleSize = -1
		vErr = -1
		vExe = ""
	...
	Private Function GetErr(ByVal Num)
		Select Case Num
			Case 0
				GetErr = "COMPLETE"
			Case 1
				GetErr = "ERROR_FILE_EXCEEDS_MAXSIZE_LIMIT(超出上限)"
			Case 2
				GetErr = "ERROR_INVALID_ENCTYPEOR_METHOD()"
			Case 3
				GetErr = "ERROR_INVALID_FILETYPE(类型出错)(." & ucase(vErrExe) & ")"
			Case 5
				GetErr = "ERROR_FILE_EXCEEDS_SIZE_LIMIT(文件大小出错)"
		End Select
	End Function
	...
	Public Function QuickSave(ByVal formname,Byval SavePath)
		...
		For Each v In Fils
			...
			If formStart = lcase(formname) Then
				Set File = Fils(v)
