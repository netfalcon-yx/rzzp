...
if uid="" then	msg "考生参数出错！","-1"
%>
</head>
<body <%if uid<>"" and instr(uid,",")>0 then rw "onload=window.print()"%>>
<div class="Noprint">
  <h4><span class="redfont">准考证打印时请使用A4纸张横向打印并取消页眉页脚</span> </h4>
</div>
<%
...
if instr(uid,",")>0 then
	sql_z="select * from tb_exam where jid="&jid&" and uid in("&uid&") order by kh"
else
	sql_z="select * from tb_exam where uid in(select uid from tb_bm where jid="&jid&" and uid="&uid&" and gzdw="&session("duid")&")"&" order by kh"
end if
