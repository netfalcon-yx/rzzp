...
if dwid<>"" then
	dwid=intsql(dwid)
	sql="select dname from tb_duser where uid="&dwid
...
<div class="Noprint">
  <h4><span class="redfont">准考证打印时请使用A4纸张横向打印</span> 【<a href="javascript:window.print()">打印</a>】 【<a href="javascript:history.back()">返回</a>】</h4>
</div>
<%
sql_all="select * from tb_exam where jid="&jid&" and uid in(select uid from tb_user where did="&dwid&") order by kh,rid"
...
while not rsall.eof
	...
	rs.close
%>
<table width="842" height="595" border="0" align="center" cellpadding="0" cellspacing="0" class="printtable" style="margin-top:10px">
  <tr>
	...
	<div class="zkzrdb">（盖章）<br /><%=year(date())&"年"&month(date())&"月"%></div>
	</td>
