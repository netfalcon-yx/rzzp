...
sql="select uname,(select dname from tb_duser where uid=a.did)as dwname from tb_user a where uid="&uid
...
fgwid2=0
if id<>"" then
	sql="select * from tb_bm where id="&id&" and uid="&uid
	rs.open sql,conn,1,1
	if rs.eof then
		msg "参数错误！","-1"
	else
...
<form id="form1" name="form1" method="post" action="admin_save_baom.asp" onSubmit="return Validator.Validate(this,1)">
  <table border="0" align="center" cellpadding="0" cellspacing="0" class="mytable" style="width:95%">
    ...
    <tr>
      ...
      <td colspan="2" bgcolor="#FFFFF7"><label>
        ...
        <input name="zyjszw2" type="text" id="zyjszw2" value="<%=zyjszw2%>" style="display:none" />
      </label></td>
    </tr>
    <tr>
